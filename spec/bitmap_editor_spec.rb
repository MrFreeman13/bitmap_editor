...
describe BitmapEditor do
  ...
  it 'should print an error if file starts with non-char symbol' do
    expect { bitmap_editor.run('spec/fixtures/incorrect_content.txt') }.to output("unrecognised command :(\n").to_stdout
  end

  it 'should pass if file has correct command' do
    expect { bitmap_editor.run('spec/fixtures/correct_content.txt') }.to output("There is no image\n").to_stdout
  end
end
